    ...
    <div class="user-layout reveal visible">
      <div class="user-grid">
        <div class="glass-card booking-form-container">
          <div class="card-header"><h3>Configure Session</h3><p>Select your parameters below</p></div>
            ...
            ${hasActive ? `
              <div class="suggestion-box">
                ...
                <div class="s-text">
                  ...
                  <p>You already have a pass for <strong>Spot ${activeSpotNum}</strong>.</p>
                </div>
              </div>
            ` : ''}
            <div class="input-row ${hasActive ? 'op-3' : ''}">
              <div class="input-group"><label class="static-label">Vehicle Registration</label><input type="text" id="vehicle-registration" class="custom-select" placeholder="e.g. ABC-1234" ${hasActive ? 'disabled' : ''} required></div>
              <div class="input-group"><label class="static-label">Vehicle Type</label><select id="vehicle-type" class="custom-select" ${hasActive ? 'disabled' : ''}><option value="Four-wheeler">Four-wheeler (Car/SUV)</option><option value="Two-wheeler">Two-wheeler (Bike)</option></select></div>
            </div>
            <div class="input-row mt-4 ${hasActive ? 'op-3' : ''}">
              <div class="input-group"><label class="static-label">Reservation Date</label><input type="date" id="booking-date" class="custom-date-input" ${hasActive ? 'disabled' : ''}></div>
              <div class="input-group"><label class="static-label">Time Slot</label><select id="booking-time" class="custom-select" ${hasActive ? 'disabled' : ''}><option value="08:00">08:00 AM</option><option value="10:00">10:00 AM</option><option value="12:00">12:00 PM</option><option value="14:00">02:00 PM</option><option value="16:00">04:00 PM</option><option value="18:00">06:00 PM</option><option value="20:00">08:00 PM</option></select></div>
            </div>
            <div id="selected-spot-display" class="selected-spot-banner mt-6">
              <div class="banner-info"><span class="label">Assigned Coordinate</span><strong id="display-spot-number">${hasActive ? (activeSpotNum || '--') : selectedSpotNum}</strong></div>
              <div class="banner-price"><span class="label">Fixed Rate</span><strong>₹100.00</strong></div>
            </div>
        ...
        <div class="glass-card active-booking-container">
          <div class="card-header"><h3>Active Digital Pass</h3><p>Authorized access token</p></div>
          <div id="active-booking-content" class="active-booking-status"><div class="loading-dots"><span></span><span></span><span></span></div></div>
        </div>
      </div>
      <div class="glass-card mt-8 map-card"><div class="card-header"><h3>Interactive Facility Map</h3><div class="map-status"><span class="status-dot green"></span> Live Sync</div></div><p class="map-hint">Click on an <span class="text-green">Available</span> spot to select your coordinate</p><div id="customer-parking-grid" class="parking-grid mt-6"></div></div>
    </div>
  `;
  if (!hasActive) {
...
function renderCustomer() {
  ...
  spots.forEach(spot => {
    ...
    if (spot.status === 'Available' && !GlobalState.myBooking) {
      ...
      if (GlobalState.myBooking) spotDiv.onclick = () => showToast("You already have an active booking.", "error");
    }
...
async function handleNewBooking(e) {
  ...
  try {
    const orderRes = await fetch(`${API_BASE_URL}/bookings/create-order`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${GlobalState.token}` } });
    const orderData = await orderRes.json();
    if (!orderRes.ok) throw new Error(orderData.error);
    const options = {
      key: 'rzp_test_SgsIzX6vUkYnwU',
      ...
      handler: async function (response) {
        try {
          const bookRes = await fetch(`${API_BASE_URL}/bookings/book`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${GlobalState.token}` }, body: JSON.stringify({ spotId: GlobalState.selectedSpotId, vehicleType, vehicleRegistration, startTime: `${date}T${time}:00`, razorpay_order_id: response.razorpay_order_id, razorpay_payment_id: response.razorpay_payment_id, razorpay_signature: response.razorpay_signature }) });
          ...
          fetchData();
        } catch (err) { showToast(err.message, "error"); btn.disabled = false; btn.innerText = `Book Spot ${getSelectedSpotNumber()}`; }
      },
      prefill: { name: GlobalState.currentUser.name, email: GlobalState.currentUser.email },
      theme: { color: "#6366f1" },
      modal: { ondismiss: function() { btn.disabled = false; btn.innerText = `Book Spot ${getSelectedSpotNumber()}`; showToast("Payment cancelled.", "info"); } }
    };
...
function updateActivePass() {
  ...
  if (myBooking && (myBooking.status === 'Booked' || myBooking.status === 'Checked-In')) {
        ...
        <div class="pass-details">
          ...
          ${myBooking.status === 'Booked' ? `
            <div class="cancel-container mt-6">
              <button class="premium-cancel-btn" onclick="cancelBooking('${myBooking._id}')"><span class="btn-text">Terminate Reservation</span><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></button>
    ...
    `;
    new QRCode(document.getElementById("active-pass-qr"), { text: myBooking.qrCodeToken, width: 180, height: 180, colorDark : "#000000", colorLight : "#ffffff" });
  } else { container.innerHTML = `<div class="empty-pass"><div class="empty-icon">🎫</div><p>No active reservations</p></div>`; }
}
...
function renderHistoryView(mainContent) {
  ...
  mainContent.innerHTML = `
    <div class="glass-card reveal visible">
      <div class="card-header"><h3>Reservation History</h3><p>Log of your previous facility interactions</p></div>
      <div class="table-wrapper mt-6">
        <table class="activity-table">
          <thead>
            <tr><th>Booked At</th><th>Check-In</th><th>Check-Out</th><th>Spot</th><th>Vehicle</th><th>Status</th></tr>
          </thead>
          <tbody>
            ${history.map(item => {
              const bookedAt = new Date(item.createdAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' });
              const checkIn = item.checkInTime ? new Date(item.checkInTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--';
              const checkOut = item.endTime ? new Date(item.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--';
              return `
                <tr>
                  <td>${bookedAt}</td>
                  <td>${checkIn}</td>
                  <td>${checkOut}</td>
                  <td><strong>${item.spot?.spotNumber || 'N/A'}</strong></td>
                  <td>${item.vehicleRegistration || '--'}</td>
                  <td><span class="badge-status ${item.status.toLowerCase()}">${item.status}</span></td>
                </tr>
              `;
            }).join('') || '<tr><td colspan="6" style="text-align:center; padding: 40px; opacity: 0.5;">No history records found</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>
  ...
  mainContent.innerHTML = `
    <div class="profile-layout glass-card reveal visible">
      <div class="profile-header">
        ...
        <h3>${user.name}</h3><p class="text-muted">Registered Member</p>
      </div>
